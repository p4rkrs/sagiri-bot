...
const isSelfbot = process.env.SELFBOT === 'true'; // eslint-disable-line no-process-env

if (isSelfbot) {
    require('./src/selfbot');
} else {
    const cluster = new Crystal(path.join('src', 'bot.js'), parseInt(process.env.PROCESS_COUNT, 10)); // eslint-disable-line no-process-env
