		...
		superagent.get(`http://rule34.xxx/index.php?page=dapi&s=post&q=index&tags=${args.tags.split(' ').join('+')}`)
			...
			.then((res) => {
				parseString(res.text, (error, result) => {
					if (error) {
						return responder.error().send('There was an error parsing to JSON for some reason...');
					}
					else if (!result.posts.post) {
						return responder.error().send('No results');
					}
					else { return responder.sucess().send(`http://${result.posts.post[Math.floor(Math.random() * result.posts.post.length)].$.file_url.substring(6)}`); }
				});
			})
			.catch(() => {
				return responder.error().send('There was an error retrieving posts.');
			});
	}
};
